...
const UploadDropzone = ({ user }: DropZoneProps) => {
    ...
    const { getRootProps, getInputProps, acceptedFiles, fileRejections, } = useDropzone({
        ...
        onDrop: async (acceptedFile, fileRejections) => {
            ...
            if (acceptedFile[0] instanceof Blob) {
                ...
                const remotePath = `/users/profile/${user.name}`; // Create the remotePath
                cdnFormData.append("remotePath", remotePath);
                cdnFormData.append("productImages[]", acceptedFile[0], acceptedFile[0].name);
...
const UploadButton = ({ user }: DropZoneProps) => {
